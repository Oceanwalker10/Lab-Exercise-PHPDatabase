...
function outputArtists() {
   try {
         ...
         while ($row = $result->fetch()) {
            echo '<li><a href="' . $_SERVER["SCRIPT_NAME"] . '?ids=' . $row['ArtistID'] . '" class="';
            if (isset($_GET['ids']) && $_GET['ids'] == $row['ArtistID']) echo 'active ';
            echo 'item">';
            echo $row['LastName'] . '</a></li>';
         }
         $pdo = null;
   }
...
function outputPaintings() {
   try {
      if (isset($_GET['ids']) && $_GET['ids'] > 0) {   
         ...
         $sql = 'select * from Paintings where ArtistId=:ids';
         $id =  $_GET['ids'];
         $statement = $pdo->prepare($sql);
         $statement->bindValue(':ids', $id);
         $statement->execute();
